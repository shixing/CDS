...
from utils.orderablebitstring import OrderableBitArray
...
class Permutation:
    ...
    def search_revert(self,bas,ba,beam_size):
        ...
        idx = bisect_left(bas,pba)
        start = max(0,idx - half_beam)
        end = min(len(bas),idx+half_beam)
        res =  bas[start:end]
        res = [self.revert(x.copy()) for x in res]
        return res
    ...
    def build_permute_index(self,num_permutation,beam_size,hamming_beam_size):
        ...
        for p in self.permutations:
            ...
            for ba in original_keys:
                c = ba.copy()
                p.permute(c)
                permuted_list.append(c)
            # sort the list
            permuted_list = sorted(permuted_list, key = lambda x : x.bin)
            self.permuted_lists.append(permuted_list)
        
    def get_neighbour_keys(self,bucket_key,k):
        query_key = OrderableBitArray(bucket_key)
        topk = set()
        for i in xrange(len(self.permutations)):
            ...
            for c in candidates:
                topk.add(c)
        topk = list(topk)
        topk = sorted(topk, key = lambda x : x.hamming_distance(query_key))
        topk_bin = [x.bin for x in topk[:k]]
        return topk_bin
